...
public class WebCrawler {
    ...
    private void test() {
        ...
        patterns.add(ahrefPattern);
        Pattern linkPattern = Pattern.compile("<link(.*?)(href)[\\\\h]*=[\\\\h]*\\\"(.*?)\\\"",  Pattern.CASE_INSENSITIVE| Pattern.DOTALL);
        patterns.add(linkPattern);
        Pattern scriptPattern = Pattern.compile("<script(.*?)(src)[\\\\h]*=[\\\\h]*\\\"(.*?)\\\"",  Pattern.CASE_INSENSITIVE| Pattern.DOTALL);
        patterns.add(scriptPattern);
        List<String> links = processPatterns(patterns);
        logger.log("total links = " + links.size());
        logger.log("links = " + links.toString().replaceAll(COMMA_SPACE, System.lineSeparator()));
    }

    private ArrayList<String> processPatterns(List<Pattern> patterns) {
